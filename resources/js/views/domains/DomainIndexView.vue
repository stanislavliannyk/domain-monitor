                        ...
                        <table class="min-w-full divide-y divide-gray-100">
                            ...
                            <tbody class="divide-y divide-gray-50 text-sm">
                                <tr v-for="domain in domains" :key="domain.id" class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        <RouterLink :to="{ name: 'domains.show', params: { id: domain.id } }"
...
async function loadPage(page = 1) {
    ...
    try {
        ...
        flash.error('Failed to load domains.')
    } finally {
        loading.value = false
    }
}
...
async function handleCheckNow(domain) {
    try {
        ...
        flash.success(`Check dispatched for "${domain.name}".`)
    } catch {
...
async function handleDelete(domain) {
    if (!confirm(`Delete "${domain.name}" and all its check history?`)) return
    try {
        await domainsApi.remove(domain.id)
        flash.success(`"${domain.name}" deleted.`)
